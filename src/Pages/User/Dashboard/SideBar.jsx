            ...
            <Box sx={{ display: 'flex' }}>
                <Navbar position="fixed" open={open}>
                    <Toolbar>
                        ...
                        {!(user?.success && user?.loggedInUser?.isAdmin === true) && <Link to={user?.success && user?.loggedInUser?.isAdmin === false ? `/cart` : '/login'}><div style={{ color: 'black', cursor: 'pointer', textDecoration: 'none' }} className='ms-5' ><Badge badgeContent={user.success && cart?.products?.length} color="info">
